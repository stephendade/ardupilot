...
void Rover::update_wheel_encoder()
{
    ...
    // send all wheel encoder delta angle and delta time to EKF
    ...
    for (uint8_t i = 0; i < g2.wheel_encoder.num_sensors(); i++) {
        ...
        const uint32_t sensor_reading_ms = g2.wheel_encoder.get_last_reading_ms(i);
        ...
        uint32_t sensor_diff_ms = sensor_reading_ms - wheel_encoder_last_reading_ms[i];
        if (sensor_diff_ms == 0 || sensor_diff_ms > 100) {
            ...
            sensor_diff_ms = now_ms - wheel_encoder_last_reading_ms[i];
            wheel_encoder_last_reading_ms[i] = now_ms;
        } else {
            wheel_encoder_last_reading_ms[i] = sensor_reading_ms;
        }
    #if HAL_NAVEKF3_AVAILABLE
        ...
        ahrs.EKF3.writeWheelOdom(delta_angle,
                            delta_time,
                            wheel_encoder_last_reading_ms[i],
                            g2.wheel_encoder.get_pos_offset(i),
                            g2.wheel_encoder.get_wheel_radius(i));
    #endif
    }
}
