...
#define TORQEEDO_LOG_TRQD_INTERVAL_MS                   1000// log TRQD message at this interval in milliseconds
...
// parameters
const AP_Param::GroupInfo AP_Torqeedo_R::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("TYPE", 1, AP_Torqeedo_R, _type, (int8_t)ConnectionType::TYPE_DISABLED, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("ONOFF_PIN", 2, AP_Torqeedo_R, _pin_onoff, -1),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("DE_PIN", 3, AP_Torqeedo_R, _pin_de, -1),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 4, AP_Torqeedo_R, _options, (int8_t)options::LOG),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("POWER", 5, AP_Torqeedo_R, _motor_power, 100),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("SLEW_TIME", 6, AP_Torqeedo_R, _slew_time, 2.0),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("DIR_DELAY", 7, AP_Torqeedo_R, _dir_delay, 1.0),

    AP_GROUPEND
};

AP_Torqeedo_R::AP_Torqeedo_R()
{
    _singleton = this;
...
void AP_Torqeedo_R::init()
{
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AP_Torqeedo_R::thread_main, void), "torqeedoR", 2048, AP_HAL::Scheduler::PRIORITY_RCOUT, 1)) {
        return;
    }
}

// initialise serial port and gpio pins (run from background thread)
bool AP_Torqeedo_R::init_internals()
{
    // find serial driver and initialise
    const AP_SerialManager &serial_manager = AP::serialmanager();
    _uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_Torqeedo_R, 0);
    if (_uart == nullptr) {
        return false;
    }
...
bool AP_Torqeedo_R::enabled() const
{
...
void AP_Torqeedo_R::thread_main()
{
    // initialisation
    if (!init_internals()) {
        return;
...
bool AP_Torqeedo_R::pre_arm_checks(char *failure_msg, uint8_t failure_msg_len)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return true;
...
const char * AP_Torqeedo_R::map_master_error_code_to_string(uint8_t code) const
{
    switch (code) {
    case 2:
...
void AP_Torqeedo_R::report_error_codes()
{
    ...
    const char* msg_prefix = "TorqeedoR:";
...
bool AP_Torqeedo_R::get_batt_info(float &voltage, float &current_amps, float &temp_C, uint8_t &pct_remaining) const
{
...
bool AP_Torqeedo_R::get_batt_capacity_Ah(uint16_t &amp_hours) const
{
...
bool AP_Torqeedo_R::parse_byte(uint8_t b)
{
...
void AP_Torqeedo_R::parse_message()
{
    ...
    if ((_type == ConnectionType::TYPE_TILLER) && (msg_addr == MsgAddress::DISPLAY)) {
        ...
        case DisplayMsgId::SYSTEM_STATE :
            if (_received_buff_len == 30) {
                ...
                if ((_options & options::LOG) != 0) {
                    // @LoggerMessage: TSTR
                    // @Description: Torqeedo System State Right
                    ...
                    AP::logger().Write("TSTR", "TimeUS,F,Err,MVolt,MCur,Pow,RPM,MTemp,BPct,BVolt,BCur", "QHBffHhBBff",
                                       AP_HAL::micros64(),
                ...
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(MAV_SEVERITY_INFO,"TSTR F:%u Err:%u MV:%4.1f MC:%4.1f P:%u MT:%d B%%:%d BV:%4.1f BC:%4.1f",
        ...
        case DisplayMsgId::SYSTEM_SETUP:
            if (_received_buff_len == 13) {
                ...
                if ((_options & options::LOG) != 0) {
                    // @LoggerMessage: TSER
                    // @Description: Torqeedo System Setup Right
                    // @Field: TimeUS: Time since system startup
                    // @Field: Flag: Flags
                    ...
                    AP::logger().Write("TSER", "TimeUS,Flag,MotType,MotVer,BattCap,BattPct,BattType,SwVer", "QBBHHBBH",
                                       AP_HAL::micros64(),
                ...
                // send to GCS
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(MAV_SEVERITY_INFO,"TSER:%u F:%u Mot:%u/%u Bat:%u/%u/%u%%",
    ...
    if ((_type == ConnectionType::TYPE_MOTOR) && (msg_addr == MsgAddress::BUS_MASTER)) {
        ...
        case MotorMsgId::PARAM:
            if (_received_buff_len == 15) {
                ...
                if ((_options & options::LOG) != 0) {
                    // @LoggerMessage: TMPR
                    // @Description: Torqeedo Motor Param Right
                    // @Field: TimeUS: Time since system startup
                    // @Field: RPM: Motor RPM
                    // @Field: Pow: Motor power
                    ...
                    AP::logger().Write("TMPR", "TimeUS,RPM,Pow,Volt,Cur,ETemp,MTemp", "QhHffff",
                                       AP_HAL::micros64(),
                                       _motor_param.rpm,
                                       _motor_param.power,
                                       _motor_param.voltage,
                ...
                // send to GCS
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(MAV_SEVERITY_INFO, "TMPR: rpm:%d p:%u V:%4.1f C:%4.1f PT:%4.1f MT:%4.1f",
        ...
        case MotorMsgId::STATUS:
            if (_received_buff_len == 6) {
                ...
                if ((_options & options::LOG) != 0) {
                    // @LoggerMessage: TMSR
                    // @Description: Torqeedo Motor Status Right
                    // @Field: TimeUS: Time since system startup
                    // @Field: State: Motor status flags
                    // @Field: Err: Motor error flags
                    AP::logger().Write("TMSR", "TimeUS,State,Err", "QBHH",
                                       AP_HAL::micros64(),
                ...
                // send to GCS
                if ((_options & options::DEBUG_TO_GCS) != 0) {
                    gcs().send_text(MAV_SEVERITY_INFO,"TMSR S:%d Err:%d",
                                   _motor_status.status_flags_value,
                                   _motor_status.error_flags_value);
                }
...
void AP_Torqeedo_R::check_for_send_end()
{
    if (_send_delay_us == 0) {
        // not sending
        return;
...
uint32_t AP_Torqeedo_R::calc_send_delay_us(uint8_t num_bytes)
{
...
void AP_Torqeedo_R::set_expected_reply_msgid(uint8_t msg_id)
{
    _reply_msgid = msg_id;
...
void AP_Torqeedo_R::check_for_reply_timeout()
{
...
void AP_Torqeedo_R::set_reply_received()
{
    _reply_wait_start_ms = 0;
}
...
bool AP_Torqeedo_R::send_message(const uint8_t msg_contents[], uint8_t num_bytes)
{
...
bool AP_Torqeedo_R::add_byte_to_message(uint8_t byte_to_add, uint8_t msg_buff[], uint8_t msg_buff_size, uint8_t &num_bytes) const
{
...
void AP_Torqeedo_R::send_motor_speed_cmd()
{
    ...
    if (!hal.util->get_soft_armed()) {
        ...
        _motor_speed_desired = constrain_int16(SRV_Channels::get_output_norm(SRV_Channel::Aux_servo_function_t::k_throttleRight) * 1000.0, -1000, 1000);
    }
...
void AP_Torqeedo_R::send_motor_msg_request(MotorMsgId msg_id)
{
    // prepare message
...
int16_t AP_Torqeedo_R::calc_motor_speed_limited(int16_t desired_motor_speed)
{
    const uint32_t now_ms = AP_HAL::millis();
...
void AP_Torqeedo_R::log_TRQD(bool force_logging)
{
    ...
    if ((_options & options::LOG) != 0) {
        // @LoggerMessage: TQDR
        // @Description: Torqeedo Status Right
        // @Field: TimeUS: Time since system startup
        // @Field: Health: Health
        ...
        AP::logger().Write("TQDR", "TimeUS,Health,DesMotSpeed,MotSpeed,SuccCnt,ErrCnt", "QBhhII",
                           AP_HAL::micros64(),
                           health,
    ...
    if ((_options & options::DEBUG_TO_GCS) != 0) {
        gcs().send_text(MAV_SEVERITY_INFO,"TQDR h:%u dspd:%d spd:%d succ:%ld err:%ld",
                (unsigned)health,
...
// send ESC telemetry
void AP_Torqeedo_R::update_esc_telem(float rpm, float voltage, float current_amps, float esc_tempC, float motor_tempC)
{
#if HAL_WITH_ESC_TELEM
    // find servo output channel
    uint8_t telem_esc_index = 0;
    IGNORE_RETURN(SRV_Channels::find_channel(SRV_Channel::Aux_servo_function_t::k_throttleRight, telem_esc_index));
...
// get the AP_Torqeedo_R singleton
AP_Torqeedo_R *AP_Torqeedo_R::get_singleton()
{
    return _singleton;
}
