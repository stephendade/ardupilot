    ...
    if ((_type == ConnectionType::TYPE_TILLER) && (msg_addr == MsgAddress::DISPLAY)) {
        ...
        case DisplayMsgId::SYSTEM_SETUP:
            if (_received_buff_len == 13) {
                ...
                gcs().send_named_float("T_L_CHG", _display_system_setup.batt_charge_pct);
            } else {
                // unexpected length
                _parse_error_count++;
            }
    ...
    if ((_type == ConnectionType::TYPE_MOTOR) && (msg_addr == MsgAddress::BUS_MASTER)) {
        ...
        case MotorMsgId::PARAM:
            if (_received_buff_len == 15) {
                ...
                gcs().send_named_float("T_L_VOLT", _motor_param.voltage);
                gcs().send_named_float("T_L_CURR", _motor_param.current);
            } else {
                // unexpected length
                _parse_error_count++;
            }
