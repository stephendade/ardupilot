...
void AP_Torqeedo_L::report_error_codes()
{
    ...
    gcs().send_named_float("T_L_ERR", _display_system_state.master_error_code);
    ...
    if ((_type == ConnectionType::TYPE_TILLER) && (msg_addr == MsgAddress::DISPLAY)) {
        ...
        case DisplayMsgId::SYSTEM_STATE :
            if (_received_buff_len == 30) {
                ...
                gcs().send_named_float("T_L_VOLT", _display_system_state.batt_voltage);
                gcs().send_named_float("T_L_CURR", _display_system_state.batt_current);
                gcs().send_named_float("T_L_CHG", _display_system_state.batt_charge_pct);
...
void AP_Torqeedo_L::log_TRQD(bool force_logging)
{
    ...
    gcs().send_named_float("T_L_HLTH", health);
}
